...
export class AlertPopup extends Container {
    ...
    private createText(): void {
        ...
        let alert: Container = PixiFactory.getText(Texts.CONFIRM_DELETE);
        alert.x = ViewPortSize.HALF_WIDTH;
        alert.y = ViewPortSize.HALF_HEIGHT;
        alert.pivot.x = alert.width * .5;
        alert.pivot.y = alert.height * .5;
        this.addChild(alert);
    }

    private createButtons(): void {
        this._confirmButton = PixiFactory.getIconButton(AtlasKeys.ICON_CONFIRM);
        this._confirmButton.x = ViewPortSize.HALF_WIDTH - this._confirmButton.width * .5 - 4;
        ...
        this._cancelButton.x = ViewPortSize.HALF_WIDTH + this._cancelButton.width * .5 + 4;
